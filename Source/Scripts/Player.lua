...
function Player:update()
	...
	local on_ice=false
	local rect = self:getCollideRect():offsetBy(self.x, self.y+1)
	local actualX, actualY, collisions, length=self:checkCollisions(rect.x, rect.y)
	if length > 0 then
		for _, s in ipairs(collisions) do
			if s.other.is_solid then
				on_ground = true
				break
			end
			if s.other.is_ice then
				on_ice = true
				break
			end
		end
	end
	print(#collisions, on_ground)
...
function Player:draw(x, y)
	...
	-- move sprite
	_, _, self.collisions, _ = self:moveTo(self.pos.x-1, self.pos.y-1)
